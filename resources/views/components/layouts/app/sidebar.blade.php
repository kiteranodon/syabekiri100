...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('メイン')" class="grid">
                <flux:navlist.item icon="home" :href="route('dashboard')" :current="request()->routeIs('dashboard')"
                    wire:navigate>{{ __('ホーム') }}</flux:navlist.item>
            </flux:navlist.group>
            ...
            <flux:navlist.group :heading="__('記録管理')" class="grid">
                ...
                <flux:navlist.item icon="beaker" :href="route('medications.index')"
                    :current="request()->routeIs('medications.*')" wire:navigate>{{ __('服薬管理') }}</flux:navlist.item>
            </flux:navlist.group>

            <flux:navlist.group :heading="__('診察関連')" class="grid">
                <flux:navlist.item icon="chart-bar" :href="route('reports.index')"
                    :current="request()->routeIs('reports.*')" wire:navigate>{{ __('レポート') }}</flux:navlist.item>
                <flux:navlist.item icon="calendar" :href="route('appointments.index')"
                    :current="request()->routeIs('appointments.*')" wire:navigate>{{ __('診察予約') }}
                </flux:navlist.item>
            </flux:navlist.group>
        </flux:navlist>
