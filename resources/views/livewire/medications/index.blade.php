...
// まとめて記入フロー完了
$completeBatchFlow = function () {
    if ($this->flow === 'batch') {
        session()->flash('success', 'まとめて記入が完了しました！今日の記録をありがとうございます。');
        session()->put('batch_completed', true); // ダッシュボードでのリフレッシュ用フラグ
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-3">
                @if ($flow === 'batch')
                    <!-- まとめて記入フロー完了ボタン -->
                    <button wire:click="completeBatchFlow"
                        class="inline-flex items-center px-6 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                        ...
                        @if ($flow === 'batch')
                            <div class="mb-4 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
                                <div class="flex items-start">
                                    <svg class="h-5 w-5 text-indigo-600 mr-2 mt-0.5" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    <div>
                                        <h4 class="text-sm font-medium text-indigo-900 mb-1">まとめて記入 (3/3) - 最後のステップ</h4>
                                        <p class="text-sm text-indigo-700">
                                            今日の服薬状況を確認し、必要に応じてチェックを入れてください。<br>
                                            完了したら右上の「記録完了」ボタンを押してまとめて記入を終了してください。
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                @if ($flow === 'batch')
                                    今日は服薬する薬がないようです。記録完了ボタンを押してまとめて記入を終了してください。
                                @else
                                    「今日の服薬」ボタンから記録を開始してください。
                                @endif
                            </p>

                            <!-- まとめて記入フロー完了ボタン（薬がない場合） -->
                            @if ($flow === 'batch')
                                <div class="mt-6">
                                    <button wire:click="completeBatchFlow"
                                        class="inline-flex items-center px-8 py-3 bg-indigo-600 border border-transparent rounded-lg font-semibold text-sm text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-lg">
